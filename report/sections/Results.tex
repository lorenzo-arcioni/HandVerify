...
configuration per split (Table \ref{tab:ours-best}) is MobileNetV3-Large + Contrastive
...
configurations (Table \ref{tab:ours-best}): EfficientNet-B1 + Contrastive on
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccccc}
    ...
    \midrule
    ResNet-18           & \textbf{0.869} & \textbf{19.2} & \textbf{1.84} & 23.3\% & 9.8\%  \\
    EfficientNet-B0     & 0.849 & 20.2 & 1.75 & 25.3\% & 10.0\% \\
    EfficientNet-B1     & 0.838 & 21.1 & 1.77 & \textbf{31.0\%} & 14.3\% \\
    MobileNetV3-Large   & 0.833 & 22.3 & 1.59 & 27.1\% & 13.1\% \\
    ResNet-34           & 0.828 & 23.1 & 1.52 & 20.2\% & 9.8\%  \\
    MobileNetV3-Small   & 0.788 & 26.3 & 1.38 & 25.0\% & \textbf{14.8\%} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[htbp]
  ...
  ResNet, orange: EfficientNet, green: MobileNetV3). Darker shade = larger variant.}
  \label{fig:radar-backbone}
\end{figure}

ResNet-18 ranks first on AUC, EER and $d'$, but has the second lowest
GAR@FAR1\% (23.3\%) and lowest GAR@FAR0.1\% (9.8\%, near-tied with ResNet-34) of all six
...
EfficientNet-B1 shows the inverse profile: third on AUC, second on $d'$ and third on
EER, but highest GAR@FAR1\% (31.0\%, $+7.7$ points over
ResNet-18).
...
GAR@FAR0.1\% (14.8\%) — an isolated-threshold effect, not a
discriminability advantage.

Within the ResNet family, ResNet-34 underperforms ResNet-18 on every
metric except GAR@FAR0.1\% (near-tied, 9.8\% vs.\ 9.8\%), despite twice
...
\toprule
    Protocol & Impostor pairs & AUC & EER & FAR@0.1\%$^\dagger$ & GAR@0.1\% & FAR@1\%$^\dagger$ & GAR@1\% \\
\midrule
    Balanced   & 4{,}051       & 0.943 & 13.4\% & 0.078\% & 8.0\% & 0.989\% & 34.0\% \\
    Exhaustive & 1{,}179{,}440 & 0.945 & 13.0\% & 0.100\% & 9.6\% & 1.000\% & 34.5\% \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
\item[$\dagger$] Effective FAR achieved at the threshold selected for the corresponding
  nominal FAR target (0.1\% or 1\%), not the nominal target itself.
\end{tablenotes}
\end{table}

The exhaustive pool yields results that are essentially in line with
the balanced protocol across all metrics (AUC $0.943\to0.945$; EER
$13.4\%\to13.0\%$; GAR@1\% $34.0\%\to34.5\%$), indicating that 1:1
balanced impostor subsampling in the main sweep was already a
reliable estimate for this configuration. At the 1\% FAR target, the
effective FAR is nearly identical between the two protocols
(0.989\% for balanced vs.\ 1.000\% for exhaustive), and GAR@1\%
tracks closely as a result ($34.0\%\to34.5\%$). At the stricter
0.1\% FAR target, the effective FAR achieved is somewhat lower for
the balanced protocol (0.078\%) than for the exhaustive one
(0.100\%): with only 4{,}051 impostor pairs, the balanced protocol
cannot finely resolve thresholds in the low-FAR regime, so the
selected threshold ends up more conservative than intended, which in
turn explains most of the gap in GAR@0.1\% ($8.0\%\to9.6\%$) — the
exhaustive pool, precisely by including far more impostor pairs, is
able to hit the 0.1\% target much more accurately. Overall, no
meaningful degradation in discriminability is observed when the
impostor pool is expanded to its full exhaustive size; the residual
differences at strict FAR targets are attributable to threshold
resolution rather than to overfitting of the balanced subsampling.
This is run for a single (backbone, loss, split) triple only, given
the cost of exhaustive scoring, and does not revisit the
...
\begin{figure}[htbp]
  ...
  lines: EER threshold. None of the 24 distributions shown pass
...
distributions are (generally) right-skewed and compressed near the similarity
...
We inspect the two most relevant failure cases for the RIMES$\to$IAM (ResNet-18 +
Contrastive) configuration, a pair-level export of the top false accepts and false
rejects.
...
Direct benchmarking against the literature is constrained by three
methodological gaps, so we position our results as a contextual reference
rather than a leaderboard ranking (Table~\ref{tab:sota}).

\emph{Task mismatch.} Most CNN work on IAM and comparable corpora addresses
closed-set identification~\cite{fiel2015,christlein2015}, reporting top-$1$
accuracy or mAP. These numbers are not commensurable with the open-set,
pairwise EER/AUC used here: identification assumes every probe has an enrolled
match and only ranks candidates, whereas verification must also reject unseen
writers at a fixed threshold. The closest methodological neighbour, the triplet
CNN of Keglevic et al.~\cite{keglevic2018}, is likewise a retrieval system
evaluated by mAP.

\emph{Protocol mismatch.} Comparable Siamese \emph{verification} studies are
typically evaluated on different scripts—Arabic, Chinese or Japanese—or under
same-dataset splits only, so neither the data distribution nor the
cross-dataset stress test we apply is reproduced. We are not aware of prior
work reporting BCE, Contrastive and Triplet objectives under one fixed training
recipe across a shared backbone set on IAM and RIMES, which is the specific
comparison our study isolates.

\emph{Operating-point reporting.} Prior studies rarely report GAR at strict
FAR; a low EER can coexist with a sharp collapse in genuine acceptance at
$\text{FAR}=0.1\%$, as our results show (Section~\ref{sec:operating-points}). This
limits comparability with the security-oriented regime and is a dimension we
make explicit.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}l L L l L@{}}
    ...
    Keglevic et al.\ (triplet CNN)~\cite{keglevic2018} & Retrieval / identification, triplet metric learning & ICDAR13 & mAP & mAP $86.1\%$ -- not comparable (task) \\
    Afzali et al.\ (CSCNN)~\cite{afzali2024} & Verification, Siamese + combined loss & IAM, IFN/ENIT & Accuracy, FAR, FRR & Partially comparable (different metrics/protocol) \\
...
Within these caveats, our best cross-dataset configuration
(RIMES$\rightarrow$IAM, EER $=8.2\%$) falls inside the range reported by the
closest comparable Siamese-verification work ($7.8$--$21\%$), and our
same-dataset result (IAM$\rightarrow$IAM, EER $=3.1\%$) is numerically lower
than that range, although a direct comparison is not warranted, since the
datasets and protocols differ.
Rather than claiming state-of-the-art, our contribution is a controlled, uniformly-evaluated
comparison: across every dataset split the metric-learning objectives dominate
BCE, and Contrastive is the most consistent—a conclusion the heterogeneous
prior literature, mixing tasks, scripts and protocols, does not allow one to
draw directly.