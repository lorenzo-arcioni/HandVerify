...
class TripletDataset(BaseWriterDataset):
    ...
    def __init__(
        ...
        print(f"  Max genuine pairs (triplets) per epoch: {max_genuine_pairs if max_genuine_pairs is not None else 'unlimited'}")
        ...
        # Genera tutte le genuine pairs (pool fisso, base per le triple E per validation)
        ...
        # Genera le triple iniziali per training (con eventuale cap sulle genuine)
        self._resample_triplets()

        # Crea validation pairs (FIXED, non cambiano ad ogni epoca, usano
        # SEMPRE tutte le genuine indipendentemente da max_genuine_pairs)
    ...
    def _resample_triplets(self, epoch: int = 0):
        """
        Genera le triple: se max_genuine_pairs e' impostato ed e' minore del
        pool totale, ricampiona prima un sottoinsieme di genuine, poi
        campiona un negativo casuale dal pool per ciascuna.
        """
        ...
        if self.max_genuine_pairs is not None and \
            ...
            current_genuine = epoch_rng.sample(self.all_genuine_pairs, self.max_genuine_pairs)
        else:
            current_genuine = self.all_genuine_pairs

        self.triplets = []

        for writer_id, anchor_path, positive_path in current_genuine:
        ...
        print(f"  Generated {len(self.triplets)} triplets "
              f"(from {len(current_genuine)}/{len(self.all_genuine_pairs)} genuine)")

    def _create_validation_pairs(self):
        """
        Crea pairs (img1, img2, label) per validation comprehensive.
        Usa tutte le genuine (mai cappate) + campiona impostors per mantenere positive_ratio.
        """
        # Genuine pairs (SEMPRE tutte, indipendentemente da max_genuine_pairs)
    ...
    def on_epoch_end(self, epoch: int):
        ...
        if (epoch + 1) % self.resample_negatives_every_n_epochs == 0:
            print(f"\n🔄 Resampling triplets for next epoch...")
